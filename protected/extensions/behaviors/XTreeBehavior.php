...
class XTreeBehavior extends CActiveRecordBehavior {
    ...
    public function getTreeItems($id = null, $showRoot = true) {
        ...
        if ($showRoot === false) {
            $models = $owner->findAll(array(
                'condition' => $this->status = 1 . ' AND ' . $this->parent_id . '=' . $rootId,
                'order' => $this->sort
            ));
            if ($models === null)
    ...
    protected function formatMenuItem($model) {
        ...
        //$url = array('site/page/'=>$this->id);
            $url = array('', 'id' => $model->getAttribute($this->id));
        $text = $model->getAttribute($this->text);
        $iconCls = $model->getAttribute($this->iconCls);
        $handler = $model->getAttribute($this->handler);
        $scope = $model->getAttribute($this->scope);
        $windowId = $model->getAttribute($this->windowId);
        $module = $model->getAttribute($this->module);
        $idMenu = $model->getAttribute($this->id);
        $type = $model->getAttribute($this->type);
        $status = $model->getAttribute($this->status);
        //$url = array('site/page/'=>$this->id);
        ...
        //return array('text'=>$label, 'handler'=>$url);
        return array('id'=>$idMenu, 'status'=>$status, 'type'=>$type, 'text' => $text, 'iconCls' => $iconCls, 'handler' => $handler, 'scope' => $scope, 'windowId' => $windowId, 'module' => $module);
    }
